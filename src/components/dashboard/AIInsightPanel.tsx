        ...
        {aiInsights.map((insight, i) => (
          ...
          >
            ...
            <div className="flex items-center gap-3 text-[10px]">
              <span className="text-primary font-medium">{insight.impact.cost}</span>
              <span className="text-muted-foreground">{insight.impact.carbon}</span>
            </div>
          </motion.div>
        ))}
      </div>

      <button className="w-full mt-3 flex items-center justify-center gap-2 text-sm font-medium text-primary hover:text-primary/80 transition-colors">
