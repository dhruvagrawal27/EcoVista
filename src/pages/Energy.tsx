...
import { AreaChart, Area, LineChart, Line, BarChart, Bar, RadarChart, Radar, PolarGrid, PolarAngleAxis, PolarRadiusAxis, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell, PieChart, Pie, Legend } from "recharts";
import { Shield, Zap, Activity, TrendingDown, TrendingUp, Building2, Cpu, Sun, Battery, AlertTriangle, Gauge, Calendar, BarChart3, Brain, FlaskConical, Thermometer, Users } from "lucide-react";
import {
  energyRiskScore, costHeatmapData, loadProfileData, benchmarkData,
  gridFlowData, equipmentLoad, forecastData, forecastAccuracy,
  buildingDetails, retrofitSuggestions
...
const Energy = () => {
  ...
  const [selectedBuilding, setSelectedBuilding] = useState(buildingDetails[0]);
  ...
  const radarData = [
    { subject: "EUI", value: selectedBuilding.eui / 1.65, fullMark: 100 },
    { subject: "HVAC", value: selectedBuilding.hvac, fullMark: 100 },
    { subject: "Carbon", value: selectedBuilding.carbon, fullMark: 100 },
    { subject: "Maintenance", value: selectedBuilding.maintenance, fullMark: 100 },
    { subject: "Occupancy", value: selectedBuilding.occupancy, fullMark: 100 },
  ];
        ...
        <Tabs defaultValue="overview" className="space-y-4">
          ...
          <TabsContent value="overview" className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <Card className="glass-card grain-overlay">
                ...
                <CardContent>
                  <div className="flex items-center gap-4 mb-4">
                    <div className="relative w-20 h-20">
                      <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                        ...
                        <circle cx="50" cy="50" r="40" fill="none" stroke="hsl(var(--chart-1))" strokeWidth="8" strokeDasharray={`${energyRiskScore.overall * 2.51} 251`} strokeLinecap="round" />
                      </svg>
                      <span className="absolute inset-0 flex items-center justify-center text-xl font-bold text-foreground">{energyRiskScore.overall}</span>
                    </div>
                    <div className="space-y-1 flex-1">
                      {energyRiskScore.factors.slice(0, 3).map(f => (
                  ...
                  <div className="flex items-center justify-between text-xs bg-accent/30 rounded-lg px-3 py-2">
                    ...
                    <span className="font-semibold text-foreground">{energyRiskScore.stabilityIndex}%</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="glass-card grain-overlay md:col-span-2">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm flex items-center gap-2"><Calendar className="w-4 h-4 text-primary" />Cost Heatmap — February</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-7 gap-1">
                    ...
                    {costHeatmapData.map((d) => {
                      const intensity = (d.cost - 15000) / 25000;
                      return (
                        <motion.div key={d.day} whileHover={{ scale: 1.15 }}
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={200}>
                    <AreaChart>
                      ...
                      <Area data={loadProfileData.weekday} dataKey="load" name="Weekday" fill="hsl(var(--chart-1) / 0.2)" stroke="hsl(var(--chart-1))" strokeWidth={2} />
                      <Area data={loadProfileData.weekend} dataKey="load" name="Weekend" fill="hsl(var(--chart-3) / 0.2)" stroke="hsl(var(--chart-3))" strokeWidth={2} />
                    </AreaChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
                    ...
                    {benchmarkData.map(b => (
                      <div key={b.metric} className="space-y-1">
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <span className="font-medium text-foreground">{b.campus} vs {b.national}</span>
                        </div>
                        <div className="flex gap-1 h-2">
                          <div className="rounded-full bg-primary" style={{ width: `${(b.campus / Math.max(b.campus, b.national)) * 100}%` }} />
                          <div className="rounded-full bg-muted-foreground/30" style={{ width: `${(b.national / Math.max(b.campus, b.national)) * 100}%` }} />
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <TabsContent value="realtime" className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {[
                { label: "Solar", value: gridFlowData.solar.current, unit: "kW", max: gridFlowData.solar.capacity, icon: Sun, color: "chart-4" },
                { label: "Battery", value: gridFlowData.battery.charge, unit: "%", max: 100, icon: Battery, color: "chart-2" },
                { label: "Grid Import", value: gridFlowData.grid.current, unit: "kW", max: 3000, icon: Zap, color: "chart-5" },
                { label: "Total Demand", value: gridFlowData.demand.current, unit: "kW", max: 4000, icon: Gauge, color: "chart-1" },
              ].map(item => (
              ...
              <Card className="glass-card grain-overlay">
                ...
                <CardContent>
                  <div className="bg-destructive/10 rounded-lg p-4 mb-3 border border-destructive/20">
                    <p className="text-sm font-semibold text-foreground">Currently {Math.round((gridFlowData.demand.current / gridFlowData.demand.predicted - 1) * 100)}% above predicted baseline</p>
                    <p className="text-xs text-muted-foreground mt-1">Predicted: {gridFlowData.demand.predicted} kW → Actual: {gridFlowData.demand.current} kW</p>
                  </div>
                  <div className="flex items-center justify-between text-xs bg-accent/30 rounded-lg px-3 py-2">
                    <span className="text-muted-foreground">Sensor Coverage</span>
                    <span className="font-semibold text-foreground">87% of buildings monitored</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="glass-card grain-overlay">
                ...
                <CardContent>
                  <div className="space-y-2 max-h-[240px] overflow-y-auto pr-1">
                    {equipmentLoad.map((eq, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs">
                        <StatusDot status={eq.status} />
                        <span className="flex-1 text-foreground truncate">{eq.name}</span>
                        <span className="text-muted-foreground">{eq.building}</span>
                        <span className="font-mono font-medium text-foreground w-16 text-right">{eq.load}/{eq.capacity} kW</span>
                      </div>
                    ))}
                  </div>
                </CardContent>
              ...
              <Card className="glass-card grain-overlay">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="bg-accent/30 rounded-lg p-3 text-center">
                    ...
                    <p className="text-xl font-bold text-foreground">{Math.round(2847 * tempMultiplier * occMultiplier / solarMultiplier).toLocaleString()} kW</p>
                  </div>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="flex gap-2 flex-wrap">
              {buildingDetails.map(b => (
                <Button key={b.id} variant={selectedBuilding.id === b.id ? "default" : "outline"} size="sm" className="rounded-full text-xs"
                  onClick={() => setSelectedBuilding(b)}>{b.name}</Button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Card className="glass-card grain-overlay">
                <CardHeader className="pb-2">
                  ...
                  <CardDescription className="text-xs">{selectedBuilding.floors} floors · {selectedBuilding.area.toLocaleString()} m² · Built {selectedBuilding.yearBuilt}</CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <Card className="glass-card grain-overlay">
                ...
                <CardContent>
                  <div className="space-y-3">
                    {retrofitSuggestions.map((r, i) => (
                      <motion.div key={i} whileHover={{ x: 4 }} className="border border-border rounded-lg p-3 space-y-1">
                        <p className="text-xs font-semibold text-foreground">{r.action}</p>
                        <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
                          <span>Cost: ₹{(r.cost / 100000).toFixed(1)}L</span>
                          <span>Saves: ₹{(r.annualSaving / 100000).toFixed(1)}L/yr</span>
                          <span>Payback: {r.payback} yrs</span>
                          <Badge variant="outline" className="text-[10px] h-4">-{r.carbonReduction} tCO₂</Badge>
                        </div>
                        <Progress value={100 / r.payback * 2} className="h-1" />
                      </motion.div>
                    ))}
                  </div>
                </CardContent>
          ...
          <TabsContent value="optimization" className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Card className="glass-card grain-overlay">
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={200}>
                    <AreaChart data={loadProfileData.weekday}>
                      <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                      <XAxis dataKey="hour" tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} />
                      <YAxis tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} />
                      <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12 }} />
                      <Area dataKey="load" fill="hsl(var(--chart-1) / 0.2)" stroke="hsl(var(--chart-1))" strokeWidth={2} />
                      <Line dataKey={() => 2200} stroke="hsl(var(--destructive))" strokeDasharray="5 3" strokeWidth={1.5} dot={false} name="Peak Limit" />
                    </AreaChart>
                  </ResponsiveContainer>
            ...
            {/* Carbon vs Cost Tradeoff */}
            <Card className="glass-card grain-overlay">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm">Carbon vs Cost Tradeoff — Pareto Frontier</CardTitle>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={250}>
                  <BarChart data={retrofitSuggestions}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                    <XAxis dataKey="action" tick={{ fontSize: 9, fill: "hsl(var(--muted-foreground))" }} angle={-15} textAnchor="end" height={60} />
                    ...
                    <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12 }} />
                    <Bar yAxisId="cost" dataKey="annualSaving" fill="hsl(var(--chart-1))" name="Annual Saving (₹)" radius={[4, 4, 0, 0]} />
                    <Bar yAxisId="carbon" dataKey="carbonReduction" fill="hsl(var(--chart-2))" name="CO₂ Reduction (t)" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
